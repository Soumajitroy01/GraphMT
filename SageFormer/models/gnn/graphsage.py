...
class GraphSAGEEncoder(nn.Module):
    """GraphSAGE-based circuit encoder with hyperparameters from Table 1."""
    ...
    def __init__(self, input_dim=4, hidden_dim=64):
        ...
        # GraphSAGE layers (Table 1: Layer1=64, Layer2=64)
    ...
    def forward(self, x, edge_index, batch):
        ...
        # print(f"After GraphSAGE layer 1: {x.shape}")
        x = self.bn1(x)
        x = F.relu(x)
        
        # Second GraphSAGE layer
        x = self.conv2(x, edge_index)
        # print(f"After GraphSAGE layer 2: {x.shape}")
        x = self.bn2(x)
        x = F.relu(x)
