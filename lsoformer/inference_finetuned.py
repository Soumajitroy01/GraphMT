...
from utils.helpers import set_seed, get_timestamp, move_batch_to_device, empty_cuda_cache
...
def inference(args):
    """Main inference function for fine-tuned model"""
    # Create directories
    Config.create_dirs()
    ...
    # First load the base LSOformer model
    base_model = LSOformer(
    ...
    # Load the ControlLSOformer model
    checkpoint = torch.load(args.model_path, map_location=device)
    
    # Create ControlLSOformer
    model = ControlLSOformer(base_model, alpha=args.alpha).to(device)
    ...
    # Save results
    results_path = os.path.join(Config.RESULTS_DIR, f"finetuned_results_{timestamp}.pkl")
    ...
    # Save to CSV
    csv_path = os.path.join(Config.RESULTS_DIR, f"finetuned_results_{timestamp}.csv")
    ...
    mape_plot_path = os.path.join(Config.VISUALIZATION_DIR, f"finetuned_mape_by_circuit_{timestamp}.png")
    ...
    if args.plot_trajectories:
        os.makedirs(os.path.join(Config.VISUALIZATION_DIR, "finetuned_trajectories"), exist_ok=True)
        ...
        for design in unique_designs[:min(5, len(unique_designs))]:
            ...
            # Plot trajectory
            traj_path = os.path.join(Config.VISUALIZATION_DIR, "finetuned_trajectories", 
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Inference with fine-tuned ControlLSOformer model")
